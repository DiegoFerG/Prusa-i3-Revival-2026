...
| KB-011 | High / Revival Intelligence & Diagnostics | Work through the practical [RID candidate backlog](../../../hardware/generation-3-enhancement-candidates.md) under the [RID subsystem rules](../../../hardware/generation-3-revival-intelligence-diagnostics.md): prototype software-only candidates first, then minimal-hardware/high-impact candidates one by one. Record benefit, complexity, dependencies, false positives/negatives, autonomy, failure modes, safety and measured results before promote/defer/reject decisions. The M01–M15 Moonshots are explicitly outside the normal implementation queue and remain production-score 1/5 research ideas unless a later decision changes that. Hardware emergency-stop topology remains an open final-safety design item. |
