...
| KB-011 | High / Revival Intelligence & Diagnostics | Work through the [RID candidate backlog](../../../hardware/generation-3-enhancement-candidates.md) under the [RID subsystem rules](../../../hardware/generation-3-revival-intelligence-diagnostics.md): prototype software-only candidates that reuse frozen hardware first, then study minimal-hardware/high-impact candidates one by one. Record benefit, complexity, dependencies, false positives/negatives where relevant, autonomy level, failure modes, safety implications and measurable results before promote/defer/reject decisions. Hardware emergency-stop topology remains an open final-safety design item. |
